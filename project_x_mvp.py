...
"""
Project X — Full single-file app (Steps 1,2,3)
...
# CONFIG
# ---------------------------
DB_FILE = os.getenv("PROJECT_X_DB", "project_x_mvp_full.db")
...
MIGRATION_COLUMNS = {
    ...
    "lost_comment": "TEXT",
    "lost_date": "TEXT",
    "qualified": "INTEGER DEFAULT 0",  # new column to track qualified leads
}
...
/* Money values (green) - we will use inline styles for numeric display */
.money { color: var(--money-green); font-weight:600; }

/* Select option hover: try to force black background and white text (may be browser dependent) */
select option:hover { background: #000000 !important; color: #ffffff !important; }

/* Priority tuning labels small */
.kv { color: var(--white); font-size:13px; }

/* Funnel palette helper classes for custom inline use */
...
class Lead(Base):
    ...
    # note: estimated_value will still live on Lead but Lead Capture no longer asks for it;
    # it's set when estimate is created or when Estimate Submitted = Yes and user supplies amount
...
def create_estimate(session, lead_id, amount, details=""):
    ...
    session.add(est)
    session.commit()
    # also update lead.estimated_value and mark estimate_submitted True
...
def compute_priority_for_lead_row(lead_row, weights):
    # value normalized (use lead.estimated_value if present)
...
def save_uploaded_file(uploaded_file, lead_id):
    """Save uploaded invoice to a local temp folder and return filename."""
    if uploaded_file is None:
        return None
    folder = os.path.join(os.getcwd(), "uploaded_invoices")
    os.makedirs(folder, exist_ok=True)
    # Use safe filename with lead id and timestamp
...
if page == "Leads / Capture":
    ...
    with st.form("lead_form"):
        ...
        with col2:
            ...
            # REPLACED field: Is the Lead Qualified?
    ...
    if not leads:
        ...
        for lead in leads:
            ...
            with st.expander(card_title, expanded=False):
                ...
                # Quick contact (override colors using inline HTML)
                qc1, qc2, qc3, qc4 = st.columns([1,1,1,4])
                phone = (lead.contact_phone or "").strip()
                email = (lead.contact_email or "").strip()
                if phone:
                    # Call (blue)
                    qc1.markdown(f"<a href='tel:{phone}'><button style='background:{getattr(st, 'call_color', '#2563eb')}; color:#000; border-radius:8px; padding:6px 10px; border: none;'>📞 Call</button></a>", unsafe_allow_html=True)
                    # WhatsApp (green)
                ...
                with st.form(form_key):
                    ...
                    with c2:
                        ...
                        # estimated value moved to pipeline; only show if estimate_submitted True
                        est_val_display = lead.estimated_value or 0.0
                        est_val_widget = st.number_input("Estimated value (USD) — (only used if Estimate Submitted = Yes)", min_value=0.0, value=float(est_val_display), step=50.0, key=f"est_{lead.id}")
                    ...
                    with f2:
                        ...
                        if estimate_sub_choice == "Yes":
                            # show amount input (if user wants to set or override)
                            ...
                            # Use est_val_widget value as estimate amount
                            est_amount_input = est_val_widget
                        else:
                            est_submitted_dt = None
                            est_amount_input = None
                    with f3:
                        # Awarded: Yes/No; if Yes -> invoice upload + comment + date
                    ...
                    if save:
                        try:
                            ...
                            # estimated value: only set if estimate_submitted True (user supplied est_val_widget)
                            if estimate_sub_choice == "Yes":
                                lead.estimated_value = float(est_val_widget) if est_val_widget else None
                            ...
                            if awarded_choice == "Yes":
                                ...
                                # save invoice file and store path
                                ...
                                # if user explicitly sets No but lead was awarded earlier, keep comment/date if provided but status respects choice
                                if lead.status == LeadStatus.AWARDED and awarded_choice == "No":
                                    # do nothing to remove past awarded data automatically
                                    pass
                            # lost handling
                            if lost_choice == "Yes":
                                lead.status = LeadStatus.LOST
                            ...
                            # qualified remains unchanged from lead capture unless changed separately (not displayed here)
                            s.add(lead)
                            s.commit()
...
# --- Page: Analytics & SLA
    ...
    if df.empty:
        ...
        # Qualified vs Unqualified chart: daily/weekly/monthly/yearly
